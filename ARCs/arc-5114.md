...
Others also suggest that the use of `Clawback Address` should be avoided, as it is believed to "...don't make sense when NFT is irreversibly bounded to the address". On the contrary, this ARC uses the clawback feature for the case where commitments, credentials or affiliations need to be revoked by the issuer. As this is not considered a token transfer, the concept of Soulbound remains intact. This is why a special primitive is suggested and described for this matter.
